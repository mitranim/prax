...
const {bind, pipe, defer, rest, spread, alter,
       ifelse, ifonly, test, testArgsAnd, and, or, not, id, di,
       slice, indexOf, foldl,
       isPrimitive, isObject, isFunction, isList,
...
pub.Atom = Atom
function Atom (state) {
  return bindTo(assign(TaskQue(), {state, watchers: {}}), Atom)
}
...
function doSwap (ref, mod, ...args) {
  ...
  if (!is(prev, next)) ref.notifyWatchers(prev, next)
}

// Like `swap` but always notifies watchers.
...
function doForceSwap (ref, mod, ...args) {
  const prev = ref.state
  const next = ref.state = mod(prev, ...args)
  ref.notifyWatchers(prev, next)
}

Atom.notifyWatchers = function notifyWatchers (ref, prev, next) {
  // Use the same `watchers` reference during iteration. If it's mutable, keys
  // may still get deleted in the process; `for..in` automatically skips them.
  const {watchers} = ref
  for (const key in watchers) watchers[key].call(null, ref, prev, next)
}

Atom.addWatcher = function addWatcher (ref, key, fun) {
  validate(isPrimitive, key)
  validate(isFunction, fun)
  ref.watchers = putIn(ref.watchers, [key], fun)
}

Atom.removeWatcher = function removeWatcher (ref, key) {
  if (key in ref.watchers) ref.watchers = putIn(ref.watchers, [key], null)
}
...
function swapInto (fun) {
  validate(isFunction, fun)
  return function swapInto_ (ref, ...args) {
    return ref.swap(fun, ...args)
  }
}
...
function linear (fun) {
  validate(isFunction, fun)
  return bind(TaskQue().enque, function lineriased () {
    return fun(...slice(arguments, 1))
  })
}
