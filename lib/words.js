...
function on (type, fun) {
  ...
  return onType(type, (state, {value, key}, app) => fun(state, value, key, app))
}

pub.one = one
function one (type, fun) {
  validate(isFunction, fun)
  return onEvent({type, key: kindaTruthy}, (state, {value, key}, app) => patchAt(
    [key],
    state,
    fun(get(state, key), value, key, app)
  ))
}
...
function manageOne (path, merge, fun) {
  return (state, event, app) => (
    merge(path, state, fun(getIn(state, path), event, app))
  )
}
