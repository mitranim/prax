...
const {get, getAt, getIn, putAt, patchAt, is} = require('emerge')
...
function upgradePatch (left, right) {
  return right == null ? undefined : patchAt([], left, right || {})
}
...
function match (pattern, effect) {
  ...
  const predicate = test(pattern)
  return function runMatch (_prev, next, event, app) {
    if (predicate(event)) return effect(event, next, app)
  }
}
