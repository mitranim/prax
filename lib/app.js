...
const {bind, pipe, ifelse, cond, foldl, flat, append, remove,
       isArray, isFunction, isPromise, validate, validateEach} = require('fpx')
const {putAt, is} = require('emerge')
...
function calc (reducers, computers, prev, event) {
  const next0 = joinReducers(reducers)(prev, event)
  const next1 = joinComputers(computers)(prev, next0)
  return next1
}

exports.calcReduce = calcReduce
function calcReduce (event, state, reducer) {
  return putAt([], state, reducer(state, event))
}

exports.calcCompute = calcCompute
function calcCompute (prev, next, computer) {
  return putAt([], next, computer(prev, next))
}

exports.foldlConverge = foldlConverge
...
exports.joinComputers = joinComputers
