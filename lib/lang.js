...
/* eslint-disable no-self-compare, no-sequences, no-return-assign */

const {keys, getPrototypeOf, prototype: objectProto} = Object
const {slice: _slice} = Array.prototype
...
function ifthen (test, func) {
  return ifelse(test, func, val(undefined))
}
...
function mapObject (func, value) {
  return keys(value).map(key => func(value[key], key))
}
...
exports.id = exports.it = id
function id (value) {
  return value
}

const val = exports.val = defer(id)
