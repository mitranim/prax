...
// Purpose: enables implicit reactivity driven by procedural data access.
// Basically an adapter for React views and espo.Reaction. Enables custom
// batching of view updates. Safely handles exceptions in lifecycle methods to
// ensure proper resource cleanup.
//
// Note on exception handling. React fails to unmount components that throw an
// exception during a lifecycle method such as `render`, `componentWillMount` or
// `componentWillReceiveProps`. This screws up resource cleanup and seems to
// sometimes cause React to get stuck in an inconsistent state, breaking it.
// Replacing exceptions with console reports circumvents the problem.
class PraxComponent extends React.PureComponent {
  ...
  // exceedingly rare for props or state to receive geniunely different
  // functions under the same key.
