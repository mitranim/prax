...
// (Hoisted because some func utils depend on this)
const Array$slice = Array.prototype.slice
...
const hof = bind(seq, bind(validate, isFunction))

const defer = exports.defer = hof(bind(bind, bind))
...
exports.spread = hof(defer(apply))
...
exports.elem = elem
function elem (value, list) {
  return !!~indexOf(value, list)
}

exports.slice = slice
function slice () {
  return Array$slice.call.apply(Array$slice, arguments)
}
...
function remove (value, list) {
  const index = indexOf(value, list)
  return ~index ? removeAt(index, list) : list
}
...
const oneOf = rest(defer(flip(elem)))
