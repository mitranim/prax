...
const {getIn, bind, adjoin, remove, isFunction, isObject, isDict, validate} = require('fpx')
...
function cachingTransformType (createClass) {
  ...
  return function transformType (type) {
    return (
      ...
      ? type.prototype.class || (type.prototype.class = createClass(funToDict(type)))
      : isDict(type)
...
const methods = {
  ...
  render () {
    const {env} = this
    try {
      const [dom, paths] = logCalls.call(this, runRender, function read () {
