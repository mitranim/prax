...
/**
 * A message bus is a fancy event system with the following characteristics:
 *   * synchronous
 *   * broadcast-only
 *   * pattern matching based
 *
 * A bus accepts single values called _messages_. Messages are identified by
 * their structure. Each listener is registered with a pattern for message
 * content; a pattern may be a function, a primitive value, or an object. Each
 * broadcast checks all pattern/listener pairs; when a pattern matches, the
 * listener is called with the message. More than one listener may be called.
 * If none match, a warning is printed.
 */
