...
const {noop, includes, isFunction} = require('fpx')
...
class PraxComponent extends React.PureComponent {
  constructor () {
    ...
    this._maybeUpdate = this.renderQue.push.bind(this.renderQue, this)
  ...
  render () {
    ...
    try {
      return this.dc.subber.run(this.subrender, this._maybeUpdate)
    }
    catch (err) {
      console.error(err)
      return null
