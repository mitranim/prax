...
function MigrateApp (app, que, mean, reducers, computers, effects) {
  if (!app) return App(que, mean, reducers, computers, effects)

  // Reset known behaviours, keeping the rest of the state unchanged.
...
function main (app, event) {
  // Data phase
  const prev = app.mean
  const mean = calc(app.reducers, app.computers, app, prev, event)
...
function calc (reducers, computers, app, prev, event) {
  const next0 = foldl(bind(reduce, app, event), prev, reducers)
  const next1 = foldlConverge(bind(compute, app, prev), next0, computers)
  return next1
}

function reduce (app, event, state, reducer) {
  return put(state, reducer(state, event, app))
}

function compute (app, prev, next, computer) {
  return put(next, computer(prev, next, app))
}
