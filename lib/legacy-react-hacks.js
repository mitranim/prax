...
const {Deconstructor} = require('espo')
  ...
  return function atomComponentProps_ (Component) {
    ...
    return {
      ...
      componentWillMount () {
        ...
        this.dc.removeRenderWatcher = atom.addWatcher(enqueUpdate.bind(this))
        this._paths = []
...
function enqueUpdate (_atom, prev, next) {
  if (cursorsChanged(this._paths, prev, next)) {
    this.dc.renderCursor = this.renderQue.push(this)
  }
}
