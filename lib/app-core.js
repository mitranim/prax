...
pub.addMod = bind(enroll, '_mods')
pub.addComp = bind(enroll, '_comps')
pub.addSub = bind(enroll, '_subs')
...
pub.enqueMethod = enqueMethod
function enqueMethod (app, asMethod) {
  validate(isFunction, asMethod)
  validate(isArray, app._pending)

  if (app._dead) return
  app._pending.push(asMethod)
