...
function where (paths, predicate, effect) {
  return (prev, next) => {
    return hasChanges(paths, predicate, checkPredicateForPath)(prev, next)
      && applyReadAll(paths, effect, next)
  }
}

function applyReadAll (paths, fn, scope) {
  return apply(fn, readAll(scope, path))
}

function hasChanges (paths, predicate, checker) {
  // return and(pathsChanged(paths), hasDifferenceBy(checker(paths, predicate))
  return (prev, next) => {
    pathsChanged(paths, prev, next)
    && hasDifferenceBy(checker(paths, predicate), next, prev)
  }
}

function hasDifferenceBy (checker, ...values) {
  return isDiff(...map(values, checker))
}
