...
const {is, or, not, bind, comp, flat, apply, defer, foldl, append, ifelse,
       remove, resolve, isArray, validate, isPromise, isFunction, isPrimitive,
       validateEach, isPlainObject} = require('./lang')
...
function reduce (event, state, func) {
  return putAt([], state, func(state, event))
}

function compute (prev, next, func) {
  return putAt([], next, func(prev, next))
}
