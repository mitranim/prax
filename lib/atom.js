...
function changedPaths (prev, next, paths) {
  return paths.filter(path => eq(prev, next, path ? path.split('.') : []))
}
...
function eq (prev, next, path) {
  ...
  // and can be compared with `===`. This allows us to skip giant tree walks
  // when watching something close to root.
  return !is(one, other)
}
